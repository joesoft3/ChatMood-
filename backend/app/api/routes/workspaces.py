...
async def email_invites(
    ...
    subject = f"👥 {user.display_name or user.email} invited you to {ws.name} on MoodAI"
    body = (
        f"Hi,\n\nYou've been invited to join the “{ws.name}” team workspace on MoodAI — "
...
async def add_member(
    ...
    if not target:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "No MoodAI account with that email — ask them to sign up first")
