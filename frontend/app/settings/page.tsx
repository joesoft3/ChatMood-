...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    if (q === "connected") setPluginMsg("✅ App connected — MoodAI can now act on it in chat (🧩 toggle).");
  ...
  async function disconnectPlugin(provider: string) {
    if (!confirm("Disconnect this app? MoodAI will lose access immediately.")) return;
  ...
  async function clearAll() {
    if (!confirm("Delete ALL of MoodAI’s memories about you?")) return;
        ...
        <div className="max-w-4xl 2xl:max-w-6xl mx-auto space-y-4">
          ...
          <Card icon={<User size={16} />} title="Account">
            {me ? (
              <div className="space-y-4 text-sm">
                <div className="space-y-1">
                  <p className="text-gray-200 font-medium">{me.display_name || "MoodAI user"}</p>
          ...
          <Card icon={<CreditCard size={16} />} title="Subscription">
            ...
            <ul className="text-xs text-gray-400 space-y-1 leading-relaxed">
              ...
              <li>🏷 <span className="text-gray-200">Watermark-free exports</span> — free renders carry a small MoodAI badge</li>
          ...
          <Card icon={<Puzzle size={16} />} title="Connected apps">
            <p className="text-xs text-gray-500">
              Let MoodAI read &amp; act in your apps. In chat, turn on the 🧩 toggle and ask —
          ...
          <div className="md:col-span-2">
            <Card icon={<Globe size={16} />} title="Custom domains">
              <p className="text-xs text-gray-500">
                Run MoodAI on your own domain for your business (white-label), or buy a new domain in real time —
          ...
          <div className="md:col-span-2">
            <Card icon={<SlidersHorizontal size={16} />} title="Custom instructions">
              <p className="text-xs text-gray-500">
                Tell MoodAI how to behave in <b>every</b> conversation — tone, format, language, expertise level
              ...
              <textarea
                ...
                rows={4}
                maxLength={2000}
                placeholder="How should MoodAI respond to you?"
          ...
          {keys && (
            <div className="md:col-span-2">
              <Card
                ...
                title="🔑 Developer API — build on MoodAI"
                action={
...
client.chat.completions.create(
    ...
    messages=[{"role": "user", "content": "Hello"}],
          ...
          <div className="md:col-span-2">
            <Card
              icon={<Brain size={16} />}
              title={`What MoodAI remembers ${mems ? `(${mems.length})` : ""}`}
            >
              <p className="text-xs text-gray-500">
                MoodAI remembers durable facts about you and what your past conversations were about — that&apos;s how
                it picks up where you left off in new chats. Delete anything you don&apos;t want MoodAI to know.
              </p>
              <div className="flex gap-2">
                <button
            ...
            <Card icon={<Trash2 size={16} className="text-red-400" />} title="Danger zone">
              <div className="space-y-3">
                <p className="text-xs text-gray-400">
                  Permanently delete your MoodAI account — chats, uploads, designs, films, edits, orders, memory,
