...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  if (!film) {
    return { title: "Film not found · MoodAI", robots: { index: false } };
  }
  const description = `${film.scenes}-scene AI film directed with MoodAI — ${
    film.audio === "voice+ambience" ? "AI voiceover + cinematic ambience" : film.audio === "voice" ? "AI voiceover" : "direction"
  }.`;
  return {
    title: `${film.title} — a MoodAI film`,
    description,
    openGraph: {
      type: "video.other",
      title: film.title,
          ...
          <>
            ...
            <div className="rounded-2xl border border-line bg-panel p-5 sm:p-6 flex flex-col sm:flex-row items-center gap-4">
              <p className="text-sm text-gray-400 text-center sm:text-left flex-1">
                ...
                <span className="text-gray-200 font-semibold">Directed with MoodAI</span> — one prompt, four model
