...
| Upgrade | Where | What |
|---|---|---|
| 🎙 **Video sound & voice** | Railway → Variables | `OPENAI_API_KEY=<key>` + `BACKEND_PUBLIC_URL=https://<your-railway-domain>` — then Video Studio's 🔊 **Voice + ambience** just works. Verify the key *before* pasting it (and the live server after) with `scripts/set-voice-key.sh --check --api https://<your-railway-domain> --token <jwt>`. Guide: [VIDEO-SOUND](VIDEO-SOUND.md) |
