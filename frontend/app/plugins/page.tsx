...
export default function PluginsPage() {
  ...
  useEffect(() => {
    ...
    if (qp === "connected") setMsg("✅ Connected — turn on 🧩 in chat to let MoodAI use it.");
  ...
  async function disconnect(p: PluginStatus) {
    if (!window.confirm(`Disconnect ${p.name}? MoodAI will lose access until you reconnect.`)) return;
    setMsg("");
    setBusy(p.provider);
    try {
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-thin">
        <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
          <StudioHero
            icon={<Puzzle size={20} />}
            title="Plugin Store"
            subtitle="Connect your accounts so MoodAI can read on demand and always ask before writing. Turn on 🧩 in chat to use them."
            actions={
              <>
